...
| 25  | Email enumeration via the prelogin lookup or Firebase Auth error codes (added Phase 2) | Firebase Auth's own sign-in already distinguishes "no such user" from "wrong password" unless the project's Email Enumeration Protection setting is enabled — turn it on. `getKdfParams` (see CRYPTOGRAPHIC_ARCHITECTURE.md §4.1) returns `null` uniformly for both cases, matching established zero-knowledge password managers. Accepted as a low-severity risk shared with any Firebase email/password product; not a blocker. |
