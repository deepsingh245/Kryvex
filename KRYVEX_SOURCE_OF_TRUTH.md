...
| Web Hosting choice resolved: Firebase, not Vercel (previously a Phase 1 TBD) — but specifically **Firebase App Hosting**, not classic Firebase Hosting's Next.js framework integration, after the latter was actually tried and abandoned (post-9w) | Project owner is deploying everything through Firebase, so Vercel was ruled out; classic Hosting's Next.js bundler shells out to plain `npm install` for `esbuild`, which can't parse this repo's `workspace:*`/`catalog:` pnpm-catalog protocols — unlike `firebase/functions` (§ this table, "no @kryvex/* workspace deps" row), `apps/web` has real runtime `@kryvex/*` dependencies that can't be stripped/mirrored the same way, so the same class of fix wasn't available. App Hosting is a separate, newer product with documented pnpm-workspace support, deployed via a GitHub-connected branch (`dev`/`main`/`prod`) rather than a local `firebase deploy` | docs/DEPLOYMENT.md §6, §7 |
