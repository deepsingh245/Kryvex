...
/**
 * Glue hook: apps/mobile-only, near-twin of
 * apps/web/src/hooks/useVaultItems.ts (see that file's doc comment for the
 * full rationale) — same @kryvex/vault pure crypto/cache layers and
 * @kryvex/sync conflict bookkeeping, differing only in Firebase init
 * (native persistence), the AsyncStorage-backed local cache, and NetInfo-
 * based connectivity.
 */
export function useVaultItems(): UseVaultItemsResult {
  ...
  const localStore = useMemo(() => createAsyncStorageItemStore(), []);
  ...
  useEffect(() => {
    ...
    // Deliberately scoped to [uid, vaultEncryptionKey]: localStore is a
    // stable useMemo, decryptDoc/getServices are pure helpers — re-running
    // on their identity would re-subscribe the listener needlessly.
  ...
  useEffect(() => {
    ...
    // Deliberately scoped to [isOnline, uid]: re-running on every
    // sync.pendingWrites change would re-fire in-flight retries.
  }, [isOnline, uid]);
