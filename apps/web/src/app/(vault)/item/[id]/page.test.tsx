...
describe("ItemDetailPage", () => {
  ...
  it("asks for confirmation before deleting, and does not delete on cancel", () => {
    ...
    setup([loginItem()], { softDeleteItem });
    render(<ItemDetailPage />);
    ...
    expect(
      screen.queryByRole("dialog", { name: "Delete this item?" }),
    ).not.toBeInTheDocument();
    expect(softDeleteItem).not.toHaveBeenCalled();
  });

  it("deletes and navigates home once confirmed", async () => {
    const softDeleteItem = vi.fn().mockResolvedValue(undefined);
    setup([loginItem()], { softDeleteItem });
    render(<ItemDetailPage />);
    ...
    await waitFor(() => expect(softDeleteItem).toHaveBeenCalledWith("item1"));
